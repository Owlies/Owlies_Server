...
static int init(lua_State *L) {
    ...
    while(retry_time < MAX_RETRY_TIMES){
        ++ retry_time;
        if(connect(context) != -1 && ping(context) != -1)
            break;
    }
...
static int setByteString(lua_State *L) {
    ...
    if(retry_time >= MAX_RETRY_TIMES) {
        ...
        if(context)
            redisFree(context);
        lua_pushlstring(L, REDIS_SET_BYTE_STRING_ERROR, strlen(*REDIS_SET_BYTE_STRING_ERROR));
        return 1;
    }
...
static int getByteString(lua_State *L) {
    ...
    if(retry_time >= MAX_RETRY_TIMES) {
        ...
        if(context)
            redisFree(context);
        lua_pushlstring(L, REDIS_GET_BYTE_STRING_ERROR, strlen(*REDIS_GET_BYTE_STRING_ERROR));
        return 1;
    }
